...
/**
 * /lms/admin/finance — 회사 단위 정산 lite 버전 (Wave 2 Step 2).
 *
 ...
 */
export default async function LmsFinancePage() {
  ...
  return (
    <PageContainer>
      <PageHeader
        title="정산 (회사 단위)"
        description="회사별 강사 그룹화 + 합계. 본격 정산 (settlements / VAT / CSV) 은 Wave 3 에서."
      />
      ...
      <div className="rounded-[var(--radius)] border border-[var(--border)] bg-[var(--secondary)] p-4 mb-6">
        <p className="text-sm text-[var(--muted-foreground)]">
          강사 개인 단위 정산은{" "}
          <Link
            href={"/admin/finance" as never}
            className="text-[var(--primary)] hover:underline"
          >
            기존 어드민
          </Link>
          에서 진행합니다. 본 페이지는 회사 단위 합계 + 회사 정보 확인용.
        </p>
      </div>
