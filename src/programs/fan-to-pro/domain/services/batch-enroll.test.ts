 ...
 *   MIN_PER_COURSE = 10. 과정별 독립. 올인원 부분개강 = 열린 과정만 enrolled +
 *   안 열린 과정분 부분환불.
 */
...
describe("resolveBatchOutcome — per-course 정원", () => {
  it("(a) 둘 다 ≥10 → 전원 enrolled (full)", () => {
    ...
    const out = resolveBatchOutcome(applicants);
  ...
  it("(d) 경계값: 정확히 10 → 열림", () => {
    const out = resolveBatchOutcome(singles("a-r", 10, "ar"));
  ...
  it("(d') 경계값: 9 → 안 열림", () => {
    const out = resolveBatchOutcome(singles("a-r", 9, "ar"));
    expect(out.runs["a-r"]).toBe(false);
    expect(out.cancelledIds).toHaveLength(9);
    expect(out.enrolledFullIds).toEqual([]);
  });

  it("MIN_PER_COURSE 상수는 10", () => {
  ...
  it("min 파라미터 오버라이드 가능", () => {
    const out = resolveBatchOutcome(singles("a-r", 5, "ar"), 5);
    expect(out.runs["a-r"]).toBe(true);
  });

  it("빈 입력 → 전부 빈 결과", () => {
    const out = resolveBatchOutcome([]);
