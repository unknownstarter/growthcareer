...
## 1기 손익 분석 (10명 paid / 1명 환불 반영, 2026-06-23 갱신)

> 2026-06-22 시점 paid 11명 → 6/23 1명 환불 처리 → **현재 paid 10명**.
> 환불자는 `applicants.status='refunded'` + `refunded_at` 토글, 회계 처리는 매출 -, 비용 + (실수령 0).

### 매출 (10 × 880,000원)

- 8,800,000원 (정가 880,000 × 10명)
- VAT 포함 (880,000 = 800,000 + VAT 80,000)
- 매출 (excl VAT) = 8,000,000원
- 매출 VAT = 800,000원

### 환불 처리 (1명, 2026-06-23)

- 매출 차감: 880,000원
- 환불 송금 비용: 880,000원 (실 송금)
- 회계: 매출 취소 / VAT 환급 / cohort_expenses 에 `other` 카테고리 entry (환불 안내 수수료 / 송금 수수료 등) — 운영자 manual entry
...
| **Cowork 마케팅 수수료** | **1,056,000** | **현 paid 10명 × 880,000 × 0.12 (환불자 차감 — 약정 확인 필요)** |
| 소계 | **7,156,000** | |

> **Cowork 수수료 산정 기준 (확인 필요)**:
> - 위 계산은 **VAT 포함 수강료 기준** (880,000 × 0.12 = 105,600 × 11 = 1,161,600)
> - 만약 **VAT 제외 수강료 기준** = 800,000 × 0.12 = 96,000 × 11 = 1,056,000
> - 만약 **paid 11명만** vs **신청자 26명 모두** — 결제한 인원당이므로 paid 11명만 (현재 계산)
> - 환불자 발생 시 차감 여부 — 약정에 따라 결정
> - 정산 시점 (월별 / cohort 종료 후) — 약정에 따라 결정
>
> → 노아 확인 필요. 본 수치는 보수 시나리오 (VAT 포함).

### 손익 (10명 기준)

- 매출 (excl VAT): 8,000,000
- 비용: 7,156,000
- **마진**: 844,000원
- 마진율: 약 10.5%
...
- 비용 1인 = 약 716,000원 (7,156,000 / 10)
- 마진 1인 = 약 84,000원

### 환불 1건 + Cowork 12% 도입 후 마진율 변화

| 시나리오 | 매출 (excl) | 비용 | 마진 | 마진율 |
|---|---|---|---|---|
| 11명 paid + Cowork 12% | 8.8M | 7.26M | 1.54M | 17.5% |
| **10명 paid + Cowork 12% (현재)** | **8.0M** | **7.16M** | **0.84M** | **10.5%** |
| 8명 (폐강 기준선) | 6.4M | 7.06M | -0.66M | -10% (적자) |

→ 환불 1건 = 마진율 약 7%p 하락. 폐강 기준 8명에 매우 가까워짐.

### 다음 기수 시 재검토 사항

- 정원 30명 가정 시: 매출 26.4M / Cowork 수수료 3.17M → 마진 약 5M (마진율 18%)
- 가격 인상 시: 1,000,000원 × 30명 = 30M, Cowork 수수료 3.6M, 마진 약 6M (마진율 20% 회복)
- **수수료율 협상**: 12% → 10% (마진 +2%p) 또는 정원 따라 tiered (소규모는 면제 / 대규모만 적용) 검토 가치
- **환불자 Cowork 수수료 차감 여부 약정 확인 필요** — 본 손익은 차감 가정 (10명 기준). 약정에 따라 11명 기준 1,161,600원일 수도 있음.
...
- `event` — 네트워킹 / 수료식
- `materials` — 자료 / 인쇄
- `marketing` — 광고 / 카드뉴스 / 배너 (직접 비용)
- `partner_commission` — **Cowork (DEEPI) 마케팅 수수료** 등 파트너 수수료 (신규 카테고리 추가 필요 — 또는 marketing 안에 `subcategory='cowork'` 분기)
- `other` — 기타

각 비용 row: `cohort_id` + `category` + `amount_krw` + `vat_included` + `description` + `created_at` + `paid_at`

**Cowork 수수료 운영**:
- 매 cohort 종료 후 paid 인원 × 12% 정산 (또는 약정 시점)
- 운영자가 직접 `/fan-to-pro/admin/finance` 에서 cohort_expenses entry 추가
- 자동화 후보 (`08-automation-candidates.md` A8 확장): 매 cohort `status=completed` 시 Cowork 수수료 자동 계산 + 송금 안내
