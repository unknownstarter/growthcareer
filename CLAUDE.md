...
- **리서치 산출물은 파일로**: 결정에 쓰이는 서브에이전트 리서치는 `docs/research/YYYY-MM-DD-<slug>.md` 저장을 프롬프트에 명시. "텍스트로만 반환" 은 즉석 확인용 단문에만 허용. 대화에만 있는 산출물은 세션이 끊기면 유실됨 (2026-09-24 사고: 완료된 SaaS 리서치를 다음 세션이 통째로 놓침). 박제: `docs/lessons/2026-09-24-research-output-lost-in-chat.md`.
...
| **Juno** | Business Model Analyst | "누가 월 얼마를 왜 내나"로 시작하는 수익성 심판. 안 되는 건 킬. |
| **Cora** | Customer Persona Reviewer | 타겟 고객으로 빙의해 1인칭으로 막히는 지점을 보고하는 검수자. |
...
### 6.1 레퍼런스 재현 전 실제 구현 검사 (2026-09-21 박제)

외부 사이트의 시각 효과/디자인을 재현할 땐 **착수 전에 그 레퍼런스가 실제로 어떻게 구현됐는지 먼저 검사**한다. Playwright DOM 검사로 `<canvas>`+WebGL 컨텍스트 / 정적 `<img>` 에셋 / three 나 spline 스크립트 / `<video>` 여부를 확인해 **"라이브 효과 vs 구운 이미지"부터 판정**. 안 하면 엉뚱한 방식(라이브 2D 필터, 이미지 생성)으로 삽질. 그리고 사용자가 지목한 "효과"와 "오브젝트"를 분리 확인. 사고 박제: `docs/lessons/2026-09-21-replicate-reference-inspect-first.md`. 구현 기법: `docs/design/webgl-glass-hero-technique.md`.

---

## 6.5–6.8 카피 · 디자인 스타일 룰 (요약 — 상세: `docs/rules/copy-and-design.md`)

사용자 노출 카피·UI 는 아래 상시 룰. 신규 카피/UI 작업 전 상세 문서 확인. **기계적 검사(부호·마침표)는 grep/린트로, LLM 은 자연스러움 판단만** (전수 파일 읽기 = 토큰 낭비).

- **§6.5 카피 부호·마침표**: em dash `—`·en dash `–`·중간점 `·`·곡선따옴표·단일 ellipsis `…` 금지 (한글·영문 공통). 짧은 UI 문구(타이틀/서브타이틀/안내문/라벨/CTA/빈상태/토스트) **끝 마침표 `.` 금지**. 문단·법률 문서는 예외. 짧은 구분자는 일반 하이픈 ` - ` 또는 `/`.
- **§6.6 금액**: 한국 원 단위 필수. `M`/`K`/`KRW` 축약 금지 (`2,520만원` O, `25.2M` X). 대화·리포트·UI·commit·ADR 전부.
- **§6.7 인터렉션**: 신규 페이지/컴포넌트 = 1)`loading.tsx` skeleton 2)등장 fade-in(stagger) 3)hover/focus transition 4)상태변경 피드백(optimistic/스피너) 5)`motion-safe:` 접근성. 인쇄/스텁 페이지는 예외.
- **§6.8 디자인 금지**: 컬러값 그라데이션 금지 + 폰트/컴포넌트 blur glow(`0 0 Npx color`) 금지. 딤(단색 alpha)·검정 하드 드롭은 OK. accent = solid 블록. (포털 컴포넌트 `data-theme` 재부여 주의.)

메모리: [[feedback_no_ai_punctuation]] [[feedback_natural_korean_copy]] [[feedback_no_color_gradient_no_glow]] [[reference_lms_portal_theme_gotcha]].

### §6.9 새 팬투프로 디자인 시스템 기준 문서

새 팬투프로(별도 도메인) 랜딩/서비스의 디자인/섹션/커리큘럼 표현은 **`docs/design/fantopro-design-reference.md` 를 기준 문서로 삼는다.** 원티드 커리어 캠프 랜딩을 실측 분석한 레퍼런스(다크 베이스 + 라이트 릴리프, 행동색과 강조색 분리, 넉넉한 행간과 음수 자간, 디스플레이 미디엄 500, 24px 라운드, 개요/상세 2단 커리큘럼, 균일 헤드샷, 스티키 CTA). 원티드 요소 중 중간점 eyebrow / 색수차 / 그라데이션 / 짧은 문구 마침표는 **우리 하우스 룰(§6.5 와 §6.8) 위반이라 차용 금지** (문서 §9 조정표 참조). 팬투프로 브랜드 = 검정 + 핑크(#ec4899, 행동색) + 남보라(#6366f1, 강조). 구현 전 이 문서의 원칙 → 토큰 시트 매핑부터.
...
## 7.45 막혔을 때 (Stuck Protocol)

> 노아 룰 (2026-09-24): "없다고 그냥 끝내면 어떻게 성장을 하냐. 너 스스로 없으면 찾아보고, 안되면 만들어서라도 해결을 해야지."

없다는 확인은 탐색의 끝이 아니다. **멈추고 노아에게 넘기기 전에 4단계를 거친다.**

1. **로컬** - 이미 있는지 확인 (`~/.claude/skills`, 프로젝트 `.claude/`, `docs/`, 이전 세션 로그)
2. **공식** - 마켓플레이스, 공식 저장소, 패키지 레지스트리
3. **웹** - 검색. 공식 소스 우선, 출처 명시
4. **제작** - 없으면 만든다. 스킬은 `skill-creator`, 도구는 `tools/` 에 직접

4단계를 다 거치고도 안 되면 그때 넘긴다. 이때 **무엇을 어디까지 시도했는지 나열**한다. "없습니다" 만 말하지 않는다.

**바로 물어도 되는 예외**: 노아 계정 권한이 필요한 작업 (결제, 도메인 구매, 외부 서비스 가입) / 되돌리기 어려운 외부 영향 / 사실이 아니라 취향이나 사업 판단인 것

모르는 것을 보고하는 게 정직한 게 아니다. **확인을 안 한 채로 못 한다고 말하는 건 정직이 아니라 게으름이다.** 사고 박제: `~/factor/docs/lessons/2026-09-24-stopping-short-instead-of-solving.md`
...
- **"나중에 정리해줄게" 식 미완결 약속이 생겼을 때 즉시** - 대화에서만 약속하면 다음 세션은 그 약속의 존재를 모름 (2026-09-24 사고)
